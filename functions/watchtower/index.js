...
const timingsDiff = (timings, key1, key2) =>
  (timings[key1] && timings[key2] && hrDiff(timings[key1], timings[key2])) ||
  -1;
...
const processTimings = function (timings) {
  return {
    ...
    readable:
      timingsDiff(timings, 'secureConnect', 'readable') ||
      timingsDiff(timings, 'connect', 'readable'),
...
handlers.http = (target, data, event, resolve, reject) => {
  ...
  request.on('error', () => {
    ...
    data.durations = processTimings(data.timings);
    data.statusCode =
      typeof data.statusCode !== 'undefined' ? data.statusCode : 0;
    clearTimeout(timeout);
    resolve(data);
  });
};
